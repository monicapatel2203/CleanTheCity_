...
// #endif
        ...
        public CGColliderEnum Collider
        {
            ...
            {
                // if (m_Collider != value)
                    m_Collider = CGColliderEnum.Mesh;
                Dirty = true;
            }
        }
        ...
        CGMeshResource writeVMeshToMesh(ref CGVMesh vmesh)
        {
            ...
            mesh = res.Prepare();
            // mesh_dup = res_dup.Prepare();
            res.gameObject.layer = Layer;
            res.gameObject.tag = "CurvyRoad";
            vmesh.ToMesh(ref mesh);
            // vmesh.ToMesh(ref mesh_dup);
            ...
            // res.Filter.GetComponent<MeshRenderer>().enabled = false;
            if (canModifyStaticFlag)
                res.Filter.gameObject.isStatic = MakeStatic;
            res.Renderer.sharedMaterials = vmesh.GetMaterials();


            // res_dup.Filter.transform.localPosition = Vector3.zero;
            // res_dup.Filter.transform.localRotation = Quaternion.identity;
            // res_dup.Filter.transform.localScale = Vector3.one;
            // res_dup.Renderer.sharedMaterials = vmesh.GetMaterials();
